...
def build_apps(tag):
  with open("config/config.yml", "r") as config_file:
    ...
    for app in config_object["apps"]:
      ...
      if not app["deploy"]:
        continue
      commands.append(f"docker build -t {app['sub_domain']} -m {app['memory']}m ./sub_projects/{app['sub_domain']}/ && docker tag {app['sub_domain']} 292ppr/{app['sub_domain']}:{tag} && docker push 292ppr/{app['sub_domain']}:{tag}")
...
def get_app_deploy_command(app_config_object, tag):
  ...
  cmd = f"sudo docker stop {sub_domain}; sudo docker pull 292ppr/{sub_domain}:{tag} && sudo nohup docker run --name={sub_domain} --rm -p {port}:8000 292ppr/{sub_domain}:{tag} &"
  return cmd
...
def deploy_apps(tag):
  with open("config/config.yml", "r") as config_file:
    ...
    cmd = f"ssh -p22 292ppr@$DEPLOY_HOST \"{docker_cmd}\""
    os.system(cmd)
